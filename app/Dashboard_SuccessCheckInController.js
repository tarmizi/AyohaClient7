...
//var resultMinusForm=
                 ...
                 Ext.getCmp('containerFloatPanel_CheckOut_NonMemberActivityList').setHeight((defHeightList+resultHeightList+10));
                 Ext.getCmp('listFloatPanel_CheckOutNonMember').setHeight((defHeightList+resultHeightList+20));
             }
...
function spawnConfettiFromCoin(popupEl, count) {
 count = count || 30;
 const colors = ['#FCD34D', '#F87171', '#A78BFA', '#34D399', '#60A5FA'];

 const card = popupEl.querySelector('.ayohaSwalCard');
 const coin = popupEl.querySelector('.ayohaSwalCoin');
 const wrap = popupEl.querySelector('.ayohaSwalConfettiWrap');
 if (!card || !coin || !wrap) return;
 ...
 // origin: coin center relative to card
 const originX = (coinRect.left - cardRect.left) + (coinRect.width / 2);
 const originY = (coinRect.top - cardRect.top) + (coinRect.height / 2);

 for (let i = 0; i < count; i++) {
   const p = document.createElement('div');
   const isDot = Math.random() < 0.25;

   p.className = 'ayohaSwalConfetti' + (isDot ? ' isDot' : '');
   p.style.backgroundColor = colors[Math.floor(Math.random() * colors.length)];
   ...
   p.style.width  = (isDot ? (size - 2) : size) + 'px';
   p.style.height = (isDot ? (size - 2) : (size + 5)) + 'px';

   p.style.setProperty('--tx', tx + 'px');
   p.style.setProperty('--ty', ty + 'px');
   p.style.setProperty('--rot', (rot + 260) + 'deg');
...
function Dashboard_SuccessCheckInController_Ayoha_CelebrationReward(checkInNo, points) {
 ...
 let subText = "Check-in successful!";
 if (checkInNo === 0) subText = "Great start! (1/3)";
 else if (checkInNo === 1) subText = "Almost there! (2/3)";
 else if (checkInNo === 2) subText = "Mission Complete! (3/3)";
   ...
   Swal.fire({
     ...
     html: `
       <div class="ayohaSwalCard">
         <div class="ayohaSwalTitle">👏 Check-in Successful!</div>
